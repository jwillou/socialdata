Initialize = function(num_locals, num_tours, demo_locals, demo_tours){
  ...
  locals$knowledge[locals$demo==1] = rnorm(length(locals$knowledge[locals$demo==1]), mean=0.5, sd=0.1)
  locals$knowledge[locals$demo==2 | locals$demo==3 ] = rnorm(length(locals$knowledge[locals$demo==3 | locals$demo==2 ]), mean=0.5, sd=0.1)
  
  tourists = data.frame(
    id = (1:num_tours) + num_locals,
    demo = sample(1:3, num_tours, replace = TRUE, prob=demo.locals),
    knowledge = rnorm(num_tours, mean=0.1, sd=0.1),
    type = "tourist"
  )
